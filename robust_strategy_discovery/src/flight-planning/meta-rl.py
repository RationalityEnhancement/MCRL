...
env_array = make_envs2(n=500) + make_envs3(n=500)
    ...
    def work(self, gamma, sess, coord, saver, train):
        ...
        with sess.as_default(), sess.graph.as_default():
            while not coord.should_stop():
                ...
                env = self.env[np.random.randint(0, 1000)]
                sP = env.reset()
                s = processState(sP)
                rnn_state = self.local_AC.state_init
                ...
                if episode_count % 500 == 0 and episode_count != 0:
                    ...
                    if self.name == 'worker_0':
                        mean_reward = np.mean(self.episode_rewards[-500:])
                        mean_length = np.mean(self.episode_lengths[-500:])
                        mean_value = np.mean(self.episode_mean_values[-500:])
                        summary = tf.Summary()
...
env = env_array[np.random.randint(0, 1000)]
load_model =False
save_model = True
n_obs = env.n_obs
...
df = pd.DataFrame(columns=['i', 'return','env_type', 'actions', 'ground_truth'])
with tf.Session() as sess:
    print('Loading Model...')
    ckpt = tf.train.get_checkpoint_state(model_path)
    ...
    if True:
        if True:
            ...
            for i in range(1000):
                ...
                df.loc[i] = [i, episode_reward,etype, actions, env.ground_truth]
                print(i, episode_reward, actions)
                resum += episode_reward


print(resum / 1000)
df.to_csv('csv/a3c.csv')
