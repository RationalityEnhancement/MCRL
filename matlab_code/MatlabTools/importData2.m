function data=importData2(filename,separator)
...
if numel(regexp(text,'\n','split'))>1
    rows=regexp(text,'\n','split');
else
    rows=regexp(text,'\r','split');
end

rows=rows(1:end);
%the first row contains the variable names used by MTurk (e.g. workerid)
var_names=regexp(rows{1},separator,'split');
...
for s=1:nr_subjects
   ...
   values_sub=regexp(rows{s+1},separator,'split');
   for v=1:length(var_names)
       if strcmp(var_names{v},'answers[question_id answer_value]')
           break;
       end
       eval(['data.',strtrim(var_names{v}),'{',int2str(s),'}=''',strtrim(values_sub{v}),''';']);
   end
   ...
   for block_nr=1:nr_blocks
       ...
       temp_values=regexp(block_str,'(?<=\:)[{NaN},{true},{false},0-9,\-,\.,\,,\[,\]]*(\]|\d)','match');
...
data.incentive_condition=[repmat(data.incentive_order(1,:)+1,[20,1]);...
                 repmat(data.incentive_order(2,:)+1,[20,1]);...
                 repmat(data.incentive_order(3,:)+1,[20,1]);...
                 repmat(data.incentive_order(4,:)+1,[20,1])];
 
             %determine if the anchors were provided or self-generated and
             %set nr_anchor_types and anchor_types accordingly.
             if isfield(data,'anchors_shuffled')
                 data.nr_anchor_types=2;
                 temp=data.anchors_shuffled;
                 anchor_values=unique(data.anchors_shuffled);
                 temp(data.anchors_shuffled==anchor_values(1))=1;
                 temp(data.anchors_shuffled==anchor_values(2))=2;
                 data.anchor_type=temp;
             else
                 data.nr_anchor_types=1;
                 data.anchor_type=ones(size(data.query_times_shuffled));
             end
