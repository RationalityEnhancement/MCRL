classdef GeneralizedProbitModel
    ...
    methods
        function m=GeneralizedProbitModel(model_nr,sigma_p)
        ...
        function X=createRegressors(model,tau_likelihood,nr_outcomes)
            ...
            elseif model.nr==18
                mu_prior=3.62; tau_prior=2.61;
                [mu_Delta,tau_Delta]=NormalNormalModel.posteriorBeliefAboutDifference(mu_prior,tau_prior,tau_likelihood,options(:,1),log(options(:,2)),options(:,3),log(options(:,4)));                
                X=repmat(sqrt(tau_Delta).*mu_Delta,[nr_subjects,1]);                
            elseif model.nr==19
                mu_prior=3.62; tau_prior=2.61;
                [mu_Delta,tau_Delta]=NormalNormalModel.posteriorBeliefAboutDifference(mu_prior,tau_prior,tau_likelihood,options(:,1),log(options(:,2)),options(:,3),log(options(:,4)));                
                X=repmat([sqrt(tau_Delta).*mu_Delta,log(options(:,2))-log(options(:,4))],[nr_subjects,1]);                
            end
        end
        ...
        function log_likelihood=logLikelihood(model,y,betas,theta)
            ...
            for i=1:length(theta)
                ...
                log_likelihood(i)=sum(log(max(eps,normcdf(X(y==1,:)*betas(i,:)'))))+sum(log(1-min(1-eps,normcdf(X(y==0,:)*betas(i,:)'))));
            end
        end
