...
%% Generate MDPs where the rewards are i.i.d.
clear
load baseline_mdp
...
cost_per_click = 1;
nr_planning_operations = [0,4,8,16];
cost_of_planning = cost_per_click * nr_planning_operations;

for v=1:numel(reward_stds)
    
std_reward= reward_stds(v);

mdps=generateIIDMDPs(baseline_mdp,mu_reward,std_reward,nr_values,nr_problems);
...
for m=1:numel(mdps)
    ...
    benefit_of_planning(m,:) = delta_R(m,:) - delta_R(m,1);
    
    for n=1:3
        n_step_planning_beneficial(m,n)=benefit_of_planning(m,n+1)>benefit_of_planning(m,n);
    end
    
end

avg_benefit_of_planning(v,:)=mean(benefit_of_planning);
std_benefit_of_planning(v,:)=std(benefit_of_planning);
p_planning_beneficial(v,:)=mean(n_step_planning_beneficial);
end

VOC_of_planning_cost1dollar = avg_benefit_of_planning - repmat(cost_of_planning,...
    [numel(reward_stds),1]);

VOC_of_planning_cost1cent = avg_benefit_of_planning - 0.01 * repmat(cost_of_planning,...
    [numel(reward_stds),1]);

VOC_of_planning_cost4dollar = avg_benefit_of_planning - 4.00 * repmat(cost_of_planning,...
    [numel(reward_stds),1]);


figure(1)
plot(reward_stds,p_planning_beneficial,'LineWidth',3)
ylabel('Prob. that n-step planning is beneficial','FontSize',18)
xlabel('STD of rewards','FontSize',18)
set(gca,'FontSize',16)
legend('1-step planning','2-step planning','3-step planning')

nr_subjects = 50;
nr_trials = 6;
N = nr_subjects*nr_trials;
sem_benefit_of_planning = std_benefit_of_planning(:,2:end)/sqrt(N);

figure(2)
errorbar(repmat(reward_stds',[1,3]),avg_benefit_of_planning(:,2:end),1.96*sem_benefit_of_planning,'LineWidth',3)
ylabel('Benefit of Planning (Mean with 95% CI)','FontSize',18)
xlabel('STD of rewards','FontSize',18)
set(gca,'FontSize',16)
legend('1-step planning','2-step planning','3-step planning')

sem_benefit_of_planning = std_benefit_of_planning(:,1:end)/sqrt(N);

fig_med_cost=figure(3)
errorbar(repmat(reward_stds',[1,4]),VOC_of_planning_cost1dollar(:,1:end),1.96*sem_benefit_of_planning,'LineWidth',3)
ylabel('VOC of Planning','FontSize',18)
xlabel('STD of rewards','FontSize',18)
set(gca,'FontSize',16)
legend('no planning','1-step planning','2-step planning','3-step planning')
title('$1.00 per click','FontSize',18)
saveas(fig_med_cost,'VOC_planning_med_cost_iid.png')

fig_low_cost=figure(4)
errorbar(repmat(reward_stds',[1,4]),VOC_of_planning_cost1cent(:,1:end),1.96*sem_benefit_of_planning,'LineWidth',3)
ylabel('VOC of Planning (95% CI for 50 subjects)','FontSize',18)
xlabel('STD of rewards','FontSize',18)
set(gca,'FontSize',16)
legend('no planning','1-step planning','2-step planning','3-step planning',...
    'Location','NorthWest')
title('$0.01 per click','FontSize',18)
saveas(fig_low_cost,'VOC_planning_low_cost_iid.png')


fig_high_cost=figure(5)
errorbar(repmat(reward_stds',[1,4]),VOC_of_planning_cost4dollar(:,1:end),...
...
legend('no planning','1-step planning','2-step planning','3-step planning',...
    'Location','NorthWest')
title('$4.00 per click','FontSize',18)
saveas(fig_high_cost,'VOC_planning_high_cost_iid.png')


for h=1:3
    betas(:,h)=regress(avg_benefit_of_planning(:,h+1),[ones(numel(reward_stds),1),reward_stds(:)])
end

delta_VOC=5;

%Determine the sigma required to make 3-step planning optimal
sigma_min_cost_1dollat = (8*1.00+delta_VOC)/(betas(2,3)-betas(2,2));
sigma_min_cost_1cent = (8*0.01+delta_VOC)/(betas(2,3)-betas(2,2));

%Determine the sigma and c required to make 0-step planning optimal
sigma_low = 1
delta_VOC_low  = 2.5
c_min = delta_VOC_low + sigma_low * betas(2,1)

%Determine sigma and c so that 2-step planning is optimal
delta_VOC = 5;
c_med=1;
%2-step planning is better than 1-step planning
