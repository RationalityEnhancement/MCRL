classdef MouselabMDPMetaMDPNIPS < MDP
    ...
    methods
        ...
        function state=updateBelief(meta_MDP,state,new_observations,more_info)
            %{
            ...
            for step=(nr_steps-1):-1:1
                ...
                for n=1:numel(nodes)
                    ...
                    if more_info
                        ...
                        state.mu_V(node.nr)=max(state.mu_Q(node.nr,node.available_actions));
                        
                        [~,state.sigma_V(node.nr)]=...
                            EVofMaxOfGaussians(state.mu_Q(node.nr,node.available_actions),...
                            state.sigma_Q(node.nr,node.available_actions));
                    end
                end
        ...
        function [next_states,p_next_states,meta_MDP]=predictNextState(meta_MDP,state,c,more_info)
                ...
                if isnan(state.observations(c.state))
                    ...
                    for v=1:meta_MDP.nr_possible_payoffs
                        next_states(v).observations(c.state)=...
                            meta_MDP.payoff_values(v);
                        next_states(v)=meta_MDP.updateBelief(next_states(v),1:meta_MDP.nr_cells,more_info);
        ...
        function action_features=extractActionFeatures(meta_MDP,state,c,fast_VOC1_approximation)
            ...
            if c.is_decision_mechanism
                ...
                %tic()
                if fast_VOC1_approximation
                    [VOC,meta_MDP]=meta_MDP.myopicVOC(state,c);
                else
                    VOC=meta_MDP.myopicVOCSlow(state,c);                    
                end
                %toc()
                %tic()
        ...
        function action_features=QFeatures(meta_MDP,state,c,fast_VOC1_approximation)
            ...
            if c.is_decision_mechanism
                ...
                if fast_VOC1_approximation
                    [VOC,meta_MDP]=meta_MDP.myopicVOC(state,c);
                     ...
                     [VOC,meta_MDP]=meta_MDP.myopicVOCSlow(state,c);
                end
                cost=meta_MDP.cost_per_click;
        ...
        function VOC=myopicVOCSlow(meta_MDP,state,c)
            
            if c.is_computation
                
                %ER_act_pre = meta_MDP.expectedRewardOfActing(state,struct('is_decision_mechanism',false));
                available_actions = state.available_actions;
                max_Q=max(state.mu_Q(state.s,available_actions));
                is_best = state.mu_Q(state.s,:) == max_Q;
                p_choose = is_best/sum(is_best).*ismember(1:4,available_actions);
                
                state = meta_MDP.updateBelief(state,1:meta_MDP.nr_cells,false);
                [plan_pre,plans_pre] = meta_MDP.makePlan(state,struct('is_decision_mechanism',false));
                
                [next_states,p_next_states,meta_MDP]=meta_MDP.predictNextState(state,c,false);
                
                delta_EV=zeros(numel(next_states),1);
                
                %For each possible observation: how much better would
                %the expected reward of acting be with it than without
                %it?
                for s=1:numel(next_states)
                    %max_posterior_EV=max(next_states(s).mu_Q(location,:));
                    
                    %decision quality after the observation
                    ER_act_post = meta_MDP.expectedRewardOfActing(next_states(s),...
                        struct('is_decision_mechanism',false));
                    
                    %quality of the decision made without that
                    %observation
                    for p=1:numel(plans_pre)
                        ERs_pre(p)=meta_MDP.evaluatePlan(plans_pre{p},next_states(s));
                    end
                    
                    ER_act_pre = mean(ERs_pre);
                    
                    delta_EV(s)=ER_act_post-ER_act_pre;
                end
            ...
            if c.is_computation
                if and(isnan(state.observations(c.state)),c.state>1)
                    ...
                    if ismember(c.state,meta_MDP.getDownStreamStates(state))
                        ...
                        mu_prior=NaN(1,4);
                        for a=1:4
                            moves(a)=struct('is_decision_mechanism',true,'decision',a);
                            
                            if ismember(a,state.available_actions)
                                state = meta_MDP.updateBelief(state,1:meta_MDP.nr_cells,false);
                                mu_prior(a)=meta_MDP.expectedRewardOfActing(state,moves(a));
                            else
                                mu_prior(a)=NaN;
                            end
                        end
        ...
        function VOC=myopicVOCMaxKnown(meta_MDP,mu_prior,a,known_alternative)
            ...
            E_R_leaf = max(meta_MDP.mean_payoff,known_alternative);
            ...
            if a==alpha
                ...
                if (E_R_leaf-known_alternative<= mu_alpha-mu_beta)
                    ...
                    VOC=normcdf(ub,meta_MDP.mean_payoff,meta_MDP.std_payoff)*...
                        (mu_beta-(mu_alpha-E_R_leaf+known_alternative))-meta_MDP.cost_per_click;
                end
                ...
                %To change the decision, the sampled value would have to be larger than
                %lb
                lb=mu_alpha-mu_prior(a)+E_R_leaf;
                
                VOC=meta_MDP.std_payoff^2*normpdf(lb,meta_MDP.mean_payoff,meta_MDP.std_payoff)-...
                    (mu_alpha-mu_prior(a)-E_R_leaf-meta_MDP.mean_payoff)*...
        ...
        function VOC=myopicVOCMaxUnknown(meta_MDP,mu_prior,a)
            %This function evaluates the VOC of inspecting a leaf cell
            %downstream of the current state where the values of the other leaf(s) is/are unknown.
            %mu_prior: prior means of returns of available actions
            %a: action about which more information is being collected
            %known_alternative: maximum of the other known leafs
            [mu_sorted,pos_sorted]=sort(mu_prior,'descend');
            ...
            E_R_leaf = meta_MDP.mean_payoff;
            
            if and(a==alpha,mu_alpha>mu_beta)
                %information is valuable if it reveals that action c is suboptimal
                ...
                %    max(0, mu_beta - (mu_alpha-E_R_leaf+ETruncatedNormal(meta_MDP.mean_payoff,meta_MDP.std_payoff,...
                %    x,inf))),lb,ub)-meta_MDP.cost_per_click;
                VOC = integral(@(x) normpdf(x,meta_MDP.mean_payoff,meta_MDP.std_payoff).*...
                    max(0, mu_beta - (mu_alpha-E_R_leaf+max(x,meta_MDP.mean_payoff))),lb,ub)-meta_MDP.cost_per_click;
                ...
                %VOC=integral(@(x) normpdf(x,meta_MDP.mean_payoff,meta_MDP.std_payoff).*...
                %    max(0, (mu_prior(a)-E_R_leaf+ETruncatedNormal(meta_MDP.mean_payoff,meta_MDP.std_payoff,...
                %    x,inf))-mu_alpha),lb,ub)-meta_MDP.cost_per_click;
                VOC=integral(@(x) normpdf(x,meta_MDP.mean_payoff,meta_MDP.std_payoff).*...
                    max(0, (mu_prior(a)-E_R_leaf+max(meta_MDP.mean_payoff,x))-...
                    mu_alpha),lb,ub)-meta_MDP.cost_per_click;
        ...
        function [plan,plans]=makePlan(meta_MDP,state,computation)
            ...
            for s=1:nr_steps
                ...
                if s==1
                    ...
                    plans_to_complete = plans(s-1,:);
                end
                
                for i=1:numel(plans_to_complete)
                    
                    plan = plans_to_complete{i};
                    current_location = 1;
                    for t=1:s-1
                        current_location=meta_MDP.object_level_MDP.nextState(current_location,plan(t));
                    end
