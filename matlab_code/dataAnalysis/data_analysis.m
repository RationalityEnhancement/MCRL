...
if strcmp(experiment_name,'1A')
    ...
    fig_test=figure()
    barwitherr([sem_rel_score_test_block],...
        [avg_rel_score_test_block]), hold on
    ...
    ylabel('Performance relative to \pi_{LC}','FontSize',20)
    %plot([0.5;10.5],repmat(optimal_performance(ic),[2,1]))
    legend('no FB','action FB','metacognitive FB','Location','SouthWest') %'\pi_{LC}'
    title('Test Block Performance in Exp 1A','FontSize',20)
...
l3=plot(1:nr_training_trials,fit_low(:,3,1),'-','LineWidth',3,'Color',[1 .5 0]),hold on %
legend({FB_types{:},'\pi_{LC}'},'Location','SouthEast')
title(['Training Block', '$',num2str(info_costs(ic)),'/click'],'FontSize',32)
ylabel('Performance relative to \pi_{LC}','FontSize',24)
xlabel('Trial Number','FontSize',24)
...
if strcmp(experiment_name,'1B')
    ...
    p_FB_test=anovan(relative_score(in_test_block),{FB_nr(in_test_block),...
        ...
        'varnames',{'Delay','Message','Trial Nr.'},'model','full');
    ...
    xlim([0.75,2])
    %ylim([0.3,0.9])
    handles(3,1)=plot([0.5,2.5],mean(rel_score_pi_star)*ones(1,2),'LineWidth',3)
    ...
    saveas(fig,[fig_dir,'TestPerformance1B.png'])
...
for c=1:numel(comparisons)
    ...
    xlim([1,10])
    xlabel('Trial Number','FontSize',24)
    ylabel('Relative Score','FontSize',24)
...
if strcmp(experiment_name,'1C')
    ...
    condition_labels = {'Demonstration','Metacognitive FB','No FB'};
    
    fig_1C=figure()
    barwitherr(sem_rel_score_test_block,avg_rel_score_test_block,'FaceColor',[0.75,0.75,0.75])
    set(gca,'XTickLabel',condition_labels,'FontSize',24)
    ylabel('Relative Score','FontSize',24)
    hold on
    plot([0.5,3.5],repmat(mean(rel_score_pi_star),[1,2]),'LineWidth',3)
