...
class MetaBanditEnv(gym.Env):
    ...
    def __init__(self, n_arm=2, max_obs=10, cost=0, constant=-1):
        ...
        self.cost = -abs(cost)
        self.init = ((1, 1),) * n_arm
        self.l = np.ones(n_arm*2)*-1
        self.h = np.ones(n_arm*2)*(max_obs+1)
        self.observation_space = spaces.Box(self.l,self.h)
        self.constant = constant

        self._arms = range(n_arm)
        self._actions = range(n_arm + 1)
        self.action_space = spaces.Discrete(n_arm + 1)

        self.term_action = self._actions[-1]
        self._max_sum = max_obs + sum(concat(self.init))
        self._reset()
        self.n_actions = n_arm + 1
        self.term_state = ((-1, -1),) * n_arm

    def _reset(self):
        self._state = self.init
        return np.ravel(np.array(self._state))
